...
function markSelfStudyCalendarCells(){
  document.querySelectorAll('.cal-cell[data-date]').forEach(cell=>{
    ...
    cell.title = done ? `${cell.dataset.date} 自習室` : cell.dataset.date;
    cell.setAttribute('aria-label', done ? `${cell.dataset.date} 自習室に行った` : cell.dataset.date);
    if(done){
...
function injectSelfStudyCalendarCheck(){
  ...
  card.className='cal-check-card';
  const label=document.createElement('label');
  label.className='list-item cal-check';
  const checkbox=document.createElement('input');
  checkbox.type='checkbox';
  checkbox.className='selfStudyCheck';
  checkbox.dataset.date=date;
  checkbox.checked=isSelfStudyDone(date);
  const text=document.createElement('span');
  text.textContent='自習室に行った';
  ...
  checkbox.onchange=async(e)=>{
    ...
    catch(err){ console.error(err); alert('自習室チェックの保存に失敗しました。通信環境をご確認ください。'); renderGoals(); }
  };
}
